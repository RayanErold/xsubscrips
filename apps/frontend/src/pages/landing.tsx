...
export default function Landing() {
  ...
  return (
    <div className="min-h-screen bg-slate-50 text-slate-900 overflow-x-hidden font-sans relative selection:bg-indigo-100 selection:text-indigo-900 pb-20">
      ...
      {/* Bento Grid Feature Showcase (White SaaS Theme) */}
      <section className="max-w-6xl mx-auto px-6 py-16 relative z-10">
        <div className="text-center mb-12">
          <Badge variant="outline" className="text-indigo-600 border-indigo-200 bg-indigo-50 px-3 py-1 text-[10px] uppercase tracking-wider font-bold rounded-full mb-2">
            🎯 Platform Features
          </Badge>
          <h2 className="text-2xl sm:text-4xl font-extrabold text-slate-900 tracking-tight">Everything You Need to Reclaim Cash Flow</h2>
          <p className="text-xs sm:text-sm text-slate-500 max-w-xl mx-auto mt-1 font-normal">
            Automated tools engineered for complete clarity and immediate software spend optimization.
          </p>
        </div>

        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 max-w-5xl mx-auto">
          
          {/* Card 1: AI Invoice Scanner */}
          <div className="md:col-span-2 rounded-2xl border border-slate-200 bg-white p-6 shadow-sm hover:shadow-md transition-all group relative overflow-hidden flex flex-col justify-between min-h-[280px]">
            <div>
              <div className="w-10 h-10 rounded-xl bg-indigo-50 border border-indigo-100 flex items-center justify-center mb-4 text-indigo-600 group-hover:bg-indigo-600 group-hover:text-white transition-all">
                <UploadCloud className="w-5 h-5 transition-all" />
              </div>
              <h3 className="font-extrabold text-lg text-slate-900 mb-1">AI Invoice & Receipt Parsing</h3>
              <p className="text-xs text-slate-500 leading-relaxed max-w-md">
                Simply drop receipt PDFs or emails into your vault. Google Gemini AI extracts exact renewal dates, price tiers, and hidden tax charges with 99.8% precision.
              </p>
            </div>

            <div className="mt-5 border border-slate-200 bg-slate-50 rounded-xl p-3 flex items-center justify-between text-xs text-slate-600 relative overflow-hidden">
              <div className="flex items-center gap-2">
                <UploadCloud className="w-4 h-4 text-indigo-600 animate-bounce" />
                <span className="font-mono text-[10px]">netflix_invoice_2026.pdf</span>
              </div>
              <div className="flex items-center gap-2">
                <span className="h-1.5 w-1.5 rounded-full bg-emerald-500 animate-ping" />
                <span className="text-[10px] text-emerald-600 font-bold">Successfully Parsed</span>
              </div>
            </div>
          </div>

          {/* Card 2: Trial Reminders */}
          <div className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm hover:shadow-md transition-all group flex flex-col justify-between min-h-[280px]">
            <div>
              <div className="w-10 h-10 rounded-xl bg-indigo-50 border border-indigo-100 flex items-center justify-center mb-4 text-indigo-600 group-hover:bg-indigo-600 group-hover:text-white transition-all">
                <Bell className="w-5 h-5 transition-all" />
              </div>
              <h3 className="font-extrabold text-lg text-slate-900 mb-1">Trial Conversion Alarms</h3>
              <p className="text-xs text-slate-500 leading-relaxed">
                Receive instant notifications 48 hours before any free trial converts into a recurring paid charge.
              </p>
            </div>

            <div className="mt-5 bg-slate-900 text-white rounded-xl p-3 shadow-sm flex gap-2.5 items-start">
              <div className="w-6 h-6 bg-white rounded flex items-center justify-center shrink-0 p-0.5">
              ...
              <div className="space-y-0.5 flex-1 text-left">
                <div className="flex justify-between items-center text-[7px] text-slate-400 font-mono">
                  <span>📱 Claude Pro Trial Alert</span>
                  <span className="text-indigo-400 font-bold">48h remaining</span>
                </div>
                <p className="text-[10px] font-bold text-white leading-tight">Anthropic Claude Pro Renewing</p>
                <p className="text-[8px] text-slate-300 font-light">$20.00/mo trial converts in 2 days.</p>
              </div>
            </div>
          </div>

          {/* Card 3: Spend Vault */}
          <div className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm hover:shadow-md transition-all group flex flex-col justify-between min-h-[300px]">
            <div>
              <div className="w-10 h-10 rounded-xl bg-indigo-50 border border-indigo-100 flex items-center justify-center mb-4 text-indigo-600 group-hover:bg-indigo-600 group-hover:text-white transition-all">
                <CreditCard className="w-5 h-5 transition-all" />
              </div>
              <h3 className="font-extrabold text-lg text-slate-900 mb-1">Centralized Software Vault</h3>
              <p className="text-xs text-slate-500 leading-relaxed">
                All software services cleanly archived with category tags, billing dates, and cost trends.
              </p>
            </div>

            <div className="mt-5 space-y-1.5">
              {[
                { name: "ChatGPT Plus", val: "$20.00", progress: "w-[80%] bg-indigo-600", img: "/logos/chatgpt.png" },
                { name: "Figma Pro", val: "$15.00", progress: "w-[50%] bg-cyan-500", img: "/logos/figma.png" },
                { name: "YouTube Premium", val: "$13.99", progress: "w-[90%] bg-red-500", img: "/logos/youtube.png" }
              ].map((item, idx) => (
                <div key={idx} className="bg-slate-50 border border-slate-200/70 rounded-lg p-2 flex items-center justify-between text-[9px] gap-2">
                  <img src={item.img} alt={item.name} className="w-4 h-4 rounded object-contain shrink-0" />
          ...
          {/* Card 4: Interactive Savings Calculator */}
          <div className="md:col-span-2 rounded-2xl border border-slate-200 bg-white p-6 shadow-sm hover:shadow-md transition-all group relative overflow-hidden flex flex-col justify-between min-h-[300px]">
            <div>
              <div className="w-10 h-10 rounded-xl bg-indigo-50 border border-indigo-100 flex items-center justify-center mb-4 text-indigo-600 group-hover:bg-indigo-600 group-hover:text-white transition-all">
                <TrendingDown className="w-5 h-5 transition-all" />
              </div>
              <h3 className="font-extrabold text-lg text-slate-900 mb-1">Interactive Savings Estimator</h3>
              <p className="text-xs text-slate-500 leading-relaxed">
                Adjust your active subscription count to calculate how much you can immediately recover.
              </p>
            </div>

            {/* Savings Calculator Widget */}
            <div className="mt-5 border border-slate-200 bg-slate-50 rounded-xl p-4 space-y-4">
              <div className="space-y-2">
                <div className="flex justify-between text-xs font-bold text-slate-900">
                  <span>How many recurring subscriptions do you pay for?</span>
                  <span className="text-indigo-600 font-mono text-xs">{subCount} subscriptions</span>
                </div>
              ...
              <div className="grid grid-cols-3 gap-2 pt-2 border-t border-slate-200/80 text-center">
                <div className="p-2 bg-white rounded-lg border border-slate-200">
                  <p className="text-[8px] text-slate-400 uppercase font-bold">Estimated Spend</p>
                  <p className="text-xs font-black text-slate-900 font-mono mt-0.5">${estimatedCost.toFixed(2)}/mo</p>
                </div>
                <div className="p-2 bg-white rounded-lg border border-slate-200">
                  <p className="text-[8px] text-slate-400 uppercase font-bold">Forgotten Waste</p>
                  <p className="text-xs font-black text-red-600 font-mono mt-0.5">${potentialSavings.toFixed(0)}/yr</p>
                </div>
                <div className="p-2 bg-indigo-50 rounded-lg border border-indigo-100">
                  <p className="text-[8px] text-indigo-600 uppercase font-bold">Potential Savings</p>
                  <p className="text-xs font-black text-indigo-700 font-mono mt-0.5">${lifetimeSavings.toFixed(0)} / 3yrs</p>
                </div>
              </div>
              ...
              <div className="pt-1">
                <Button 
                  ...
                  className="w-full bg-indigo-600 hover:bg-indigo-700 text-white font-bold text-xs py-2.5 rounded-xl shadow-md gap-2"
                >
                  Claim Your ${potentialSavings.toFixed(0)}/yr Savings — Get Started Free <ArrowRight className="w-3.5 h-3.5" />
                </Button>
              </div>
            </div>
          </div>
